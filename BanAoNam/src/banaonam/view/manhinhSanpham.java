...
public class manhinhSanpham extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        tblQuanLySanPham.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã Sản Phẩm ", "Tên Sản Phẩm", "size", "Mã SPCT", "Màu Sắc", "Chất Liệu Vải", "Đơn Giá", "Số Lượng", "Link ảnh"
            }
        ) {
            boolean[] canEdit = new boolean [] {
    ...
    private void btnSuaCTSPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaCTSPActionPerformed
        ...
        int tron = JOptionPane.showConfirmDialog(this, "bạn có chắc muốn sửa chi tiết sản phẩm Mã " + lblAnh.getText() + " " + "không ?", "SỬA CTSP", JOptionPane.YES_NO_OPTION);
        if (tron == 1) {
            return;
        } else {
            if (txtMaSanPham.getText().trim().length() > 0 && txtTenSanPham.getText().trim().length() > 0) {
    ...
    private void mouclickSanPhamchitiet() {
        ...
        lblAnh.setIcon(ResizeImage(tblQuanLySanPham.getValueAt(row, 8).toString()));
